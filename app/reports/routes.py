...
from app.models import Event, Artist, EventType, ArtistType
from app import db
...
def dashboard_data():
    ...
    # Artistas por tipo
    artist_types_data = db.session.query(
        ArtistType.name,
        ArtistType.color,
        ArtistType.icon,
        db.func.count(Artist.id).label('count')
    ).join(Artist).filter(
        Artist.is_active == True
    ).group_by(ArtistType.id, ArtistType.name, ArtistType.color, ArtistType.icon).all()
    
    artist_type_names = [row.name for row in artist_types_data]
    artist_type_counts = [row.count for row in artist_types_data]
    artist_type_colors = [row.color for row in artist_types_data]
    artist_type_icons = [row.icon for row in artist_types_data]
    
    # Eventos por tipo de artista
    events_by_artist_type = db.session.query(
        ArtistType.name,
        ArtistType.color,
        db.func.count(Event.id).label('count')
    ).select_from(ArtistType).join(Artist, ArtistType.id == Artist.artist_type_id).join(Event, Artist.id == Event.artist_id).filter(
        Event.start_datetime.between(start_date, end_date)
    ).group_by(ArtistType.id, ArtistType.name, ArtistType.color).all()
    
    events_artist_type_names = [row.name for row in events_by_artist_type]
    events_artist_type_counts = [row.count for row in events_by_artist_type]
    events_artist_type_colors = [row.color for row in events_by_artist_type]
    ...
    return jsonify({
        ...
        'artists_by_type': {
            'types': artist_type_names,
            'counts': artist_type_counts,
            'colors': artist_type_colors,
            'icons': artist_type_icons
        },
        'events_by_artist_type': {
            'types': events_artist_type_names,
            'counts': events_artist_type_counts,
            'colors': events_artist_type_colors
        }
    })
